...
const useRoutineStore = defineStore("routine", {
    ...
    actions: {
        ...
        async addRoutine(name, detail, isPublic, category, difficulty){
            const result = await routineApi.add(new Routine(name, detail, isPublic, category, difficulty));
            this.items.push(result);
            return result;
        },
        async getRoutine(fullRoutine){
            const index = this.items.findIndex(fullRoutine);
            if (index >= 0){
                return this.items[index];
            }
            const result = await routineApi.get(fullRoutine.id);
            this.items.push(result);
            return result;
        },
        async getAllRoutine(){
...
class FullRoutine{
    constructor(id, name, detail, isPublic, category, difficulty, user, metadata){
        this.id=id;
        this.name=name;
        this.detail=detail;
