<template>
  <div>
      <div  class="font-weight-thin text-h6">Resultados de "{{ this.$route.query.query }}" </div>
      <v-spacer></v-spacer>
      ...
      <v-flex class="mb-n1 py-1 d-flex flex-row-reverse" align-self-end>
          ...
          <v-card flat style="background-color: transparent; margin: 0" class="mr-12" v-show="expand">
          <v-card-text class="white--text px-0">
      ...
      <v-card color="#252525" :rounded="'lg'" flat>
          ...
          <div class="my-2">
              <SelectButton v-for="categoria in categorias" :key="categoria.text" vue-style="mr-2 my-1"
                          :toggle="categoria.toggle" @click="categoria.toggle=!categoria.toggle; categoria.toggle? categoriasArray.push(categoria.text) :categoriasArray.splice(categoriasArray.indexOf(categoria.text),1)">
          ...
          <div class="my-2">
              <SelectButton v-for="grupo in gruposMusculares" :key="grupo.text" vue-style="mr-2 my-1"
              :toggle="grupo.toggle" @click="grupo.toggle=!grupo.toggle; grupo.toggle? muscularesArray.push(grupo.text) :muscularesArray.splice(muscularesArray.indexOf(grupo.text),1)">{{grupo.text}}</SelectButton>
          </div>
          </v-flex>
          ...
          <v-flex class="px-8 py-4" md3 wrap>
              ...
              <div class="my-2">
              <div class="my-2">
                  <SelectButton v-for="dificultad in dificultades" :key="dificultad.text" vue-style="mr-2 my-1"
                              :toggle="dificultad.toggle" @click="dificultad.toggle=!dificultad.toggle; dificultad.toggle? dificultadesArray.push(dificultad.text) :dificultadesArray.splice(dificultadesArray.indexOf(dificultad.text),1)">
      ...
      <v-flex class="mt-n4 d-flex flex-row-reverse" align-self-end v-if="expand">
